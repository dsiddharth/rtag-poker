...
export class RtagClient {
  ...
  public startRound(request: IStartRoundRequest): Promise<string | undefined> {
    const msgId = Math.random().toString(36).substring(2);
    this.socket.send(encode({ method: "startRound", msgId, args: request }, { ignoreUndefined: true }));
    return new Promise((resolve) => {
      this.callbacks[msgId] = resolve;
    });
  }
